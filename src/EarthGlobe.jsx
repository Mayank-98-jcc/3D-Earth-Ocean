...
const R = 2, MIN = 3.1, MAX = 9, DEFAULT = 5.4
const textures = {
...
export default function EarthGlobe() {
  const mount = useRef(null), state = useRef({}), [loading, setLoading] = useState(true), [note, setNote] = useState('Loading planet textures…'), [spinning, setSpinning] = useState(true), [borderError, setBorderError] = useState(false)
  ...
  useEffect(() => {
    ...
    const earth = new THREE.Mesh(new THREE.SphereGeometry(R, 96, 96), new THREE.MeshPhongMaterial({ color: 0x1b3a52, emissive: 0x0a1622, shininess: 5 })); globe.add(earth, atmosphere()); const lineGroup = new THREE.Group(); globe.add(lineGroup)
    state.current = { target: { x: .15, y: 0 }, current: { x: .15, y: 0 }, velocity: { x: 0, y: 0 }, distance: DEFAULT, dragging: false, last: {}, auto: true }
    const pointer = (e) => e.touches ? e.touches[0] : e
    const down = (e) => { const s = state.current, p = pointer(e); s.dragging = true; s.velocity = { x: 0, y: 0 }; s.last = { x: p.clientX, y: p.clientY } }
    const move = (e) => { const s = state.current; if (!s.dragging) return; const p = pointer(e), dx = p.clientX - s.last.x, dy = p.clientY - s.last.y; s.last = { x: p.clientX, y: p.clientY }; s.target.y += dx * .005; s.target.x = Math.max(-Math.PI / 2.1, Math.min(Math.PI / 2.1, s.target.x + dy * .005)); s.velocity = { x: dy * .005, y: dx * .005 } }
    ...
    const dom = renderer.domElement; dom.addEventListener('pointerdown', down); window.addEventListener('pointermove', move); window.addEventListener('pointerup', up); dom.addEventListener('wheel', wheel, { passive: false }); dom.addEventListener('touchstart', touchStart, { passive: true }); dom.addEventListener('touchmove', touchMove, { passive: false }); dom.addEventListener('touchend', touchEnd)
    ...
    let frame; const animate = () => { frame = requestAnimationFrame(animate); const s = state.current; if (!s.dragging) { s.target.y += s.velocity.y; s.target.x = Math.max(-Math.PI / 2.1, Math.min(Math.PI / 2.1, s.target.x + s.velocity.x)); s.velocity.x *= .94; s.velocity.y *= .94; if (s.auto && Math.abs(s.velocity.y) < .0005) s.target.y += .0018 } s.current.x += (s.target.x - s.current.x) * .12; s.current.y += (s.target.y - s.current.y) * .12; globe.rotation.set(s.current.x, s.current.y, 0); camera.position.z += (s.distance - camera.position.z) * .08; camera.lookAt(0, 0, 0); renderer.render(scene, camera) }; animate()
    ...
    return () => { cancelAnimationFrame(frame); observer.disconnect(); dom.removeEventListener('pointerdown', down); window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', up); dom.removeEventListener('wheel', wheel); dom.removeEventListener('touchstart', touchStart); dom.removeEventListener('touchmove', touchMove); dom.removeEventListener('touchend', touchEnd); node.removeChild(dom); renderer.dispose() }
  }, [])
  useEffect(() => { state.current.auto = spinning }, [spinning])
  return <main className="earth"><div ref={mount} className="canvas"/><header><strong>EARTH // LIVE VIEW</strong><small>drag to rotate · scroll to zoom</small></header><div className="controls"><button onClick={() => setSpinning((v) => !v)}>{spinning ? 'PAUSE SPIN' : 'RESUME SPIN'}</button><button onClick={reset}>RESET VIEW</button></div>{loading && <div className="loading"><i/><span>{note}</span></div>}{borderError && !loading && <p className="border-error">country border data unavailable — showing terrain only</p>}</main>
}

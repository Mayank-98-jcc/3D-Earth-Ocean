...
const R = 2, MIN = 2.6, MAX = 9, DEFAULT = 5.4
const textures = {
...
const indiaStateBorderUrls = ['https://raw.githubusercontent.com/geohacker/india/master/state/india_state.geojson', 'https://cdn.jsdelivr.net/gh/geohacker/india@master/state/india_state.geojson']
...
export default function EarthGlobe() {
  ...
  useEffect(() => {
    ...
    const earth = new THREE.Mesh(new THREE.SphereGeometry(R, 256, 128), new THREE.MeshPhongMaterial({ color: 0x1b3a52, emissive: 0x0a1622, shininess: 5 })); globe.add(earth, atmosphere()); const lineGroup = new THREE.Group(), indiaStateGroup = new THREE.Group(); indiaStateGroup.visible = false; globe.add(lineGroup, indiaStateGroup)
    ...
    const move = (e) => { const s = state.current; if (!s.dragging) return; const p = pointer(e), dx = p.clientX - s.last.x, dy = p.clientY - s.last.y; if (Math.abs(dx) + Math.abs(dy) > 2) s.moved = true; s.last = { x: p.clientX, y: p.clientY }; s.target.y += dx * .0028; s.target.x = Math.max(-Math.PI / 2.1, Math.min(Math.PI / 2.1, s.target.x + dy * .0028)); s.velocity = { x: dy * .0028, y: dx * .0028 } }
    const up = () => { state.current.dragging = false }, wheel = (e) => { e.preventDefault(); const s = state.current; s.distance = Math.max(MIN, Math.min(MAX, s.distance + e.deltaY * .001)) }
    const dist = (touches) => Math.hypot(touches[0].clientX - touches[1].clientX, touches[0].clientY - touches[1].clientY)
    const touchStart = (e) => { const s = state.current; if (e.touches.length === 2) { s.pinch = dist(e.touches); s.pinchCamera = s.distance; s.dragging = false } else down(e) }, touchMove = (e) => { const s = state.current; if (e.touches.length === 2 && s.pinch) { e.preventDefault(); const scale = s.pinch / dist(e.touches), softenedScale = 1 + (scale - 1) * .5; s.distance = Math.max(MIN, Math.min(MAX, s.pinchCamera * softenedScale)) } else move(e) }, touchEnd = () => { state.current.pinch = null; up() }
    ...
    ;(async () => { for (const url of indiaStateBorderUrls) { try { const response = await fetch(url); if (response.ok) { const stateBoundaries = await response.json(); indiaStateGroup.add(borders(stateBoundaries)); labelManager.setStateBoundaries(stateBoundaries); return } } catch {} } console.warn('Indian state boundary data unavailable') })()
    let frame; const animate = () => { frame = requestAnimationFrame(animate); const s = state.current; if (!s.dragging) { s.target.y += s.velocity.y; s.target.x = Math.max(-Math.PI / 2.1, Math.min(Math.PI / 2.1, s.target.x + s.velocity.x)); s.velocity.x *= .94; s.velocity.y *= .94; if (s.auto && Math.abs(s.velocity.y) < .0005) s.target.y += .0018 } s.current.x += (s.target.x - s.current.x) * .12; s.current.y += (s.target.y - s.current.y) * .12; globe.rotation.set(s.current.x, s.current.y, 0); camera.position.z += (s.distance - camera.position.z) * .08; camera.lookAt(0, 0, 0); indiaStateGroup.visible = camera.position.z < 4.4; labelManager.update(camera, camera.position.z); if (selectedPoint) { scene.updateMatrixWorld(); const worldPoint = globe.localToWorld(selectedPoint.clone()), projected = worldPoint.clone().project(camera), frontFacing = worldPoint.normalize().dot(camera.position.clone().normalize()) > 0; marker.hidden = !frontFacing || projected.z < -1 || projected.z > 1; if (!marker.hidden) { marker.style.left = `${(projected.x + 1) * .5 * node.clientWidth}px`; marker.style.top = `${(-projected.y + 1) * .5 * node.clientHeight}px` } } renderer.render(scene, camera) }; animate()

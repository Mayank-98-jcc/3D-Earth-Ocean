...
export default function EarthGlobe() {
  ...
  useEffect(() => {
    ...
    const marker = new THREE.Mesh(new THREE.SphereGeometry(.028, 16, 16), new THREE.MeshBasicMaterial({ color: 0xffd66b })); marker.visible = false; globe.add(marker)
    ...
    return () => { cancelAnimationFrame(frame); observer.disconnect(); dom.removeEventListener('pointerdown', down); window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', up); dom.removeEventListener('click', selectCoordinates); dom.removeEventListener('wheel', wheel); dom.removeEventListener('touchstart', touchStart); dom.removeEventListener('touchmove', touchMove); dom.removeEventListener('touchend', touchEnd); node.removeChild(dom); renderer.dispose() }
  }, [])
